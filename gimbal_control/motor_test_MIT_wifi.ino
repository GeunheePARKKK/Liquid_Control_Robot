/*
 * motor_test_MIT_wifi.ino
 * -----------------------------------------------------------------------------
 * MIT 모드 짐벌모터 2축 연속 왕복 테스트 — 와이파이판.
 *
 * motor_test_MIT.ino 와 동작이 같고, 보드가 와이파이 AP 를 띄워 TCP 로도
 * 같은 로그·명령을 주고받는다. USB 시리얼도 그대로 된다.
 *   SSID Liquid_Robot / 비밀번호 liquid1234 / 192.168.4.1 포트 23
 *   python wifi_monitor.py  로 붙으면 시리얼 모니터와 같다.
 * CAN 드라이버는 와이파이보다 먼저 설치하고 결과를 부팅 배너에 찍는다.
 *
...
#define WIFI_PASS   "liquid1234"
#define TCP_PORT    23
#define OTA_HOST    "liquid-robot-test"

WiFiServer tcpServer(TCP_PORT);
WiFiClient tcpClient;
bool canOk = false;

/* USB 와 TCP 에 같이 쓴다. TCP 는 비차단(MSG_DONTWAIT) — 상대가 느리면 그 줄은 버린다. */
class DualOut : public Print {
 public:
  size_t write(uint8_t c) override { return write(&c, 1); }
  size_t write(const uint8_t *b, size_t n) override {
    Serial.write(b, n);
    if (tcpClient && tcpClient.connected()) send(tcpClient.fd(), b, n, MSG_DONTWAIT);
    return n;
  }
};
DualOut Out;
...
void drainCAN() {
  ...
  if (fabsf(ao) > lim || fabsf(ai) > lim) {
    Out.printf("!! 폭주 차단 — 실제 위치 바깥=%.1f° 안쪽=%.1f° (한계 ±%.0f°)\n", ao, ai, lim);
    testStop("위치 한계 초과");
    return;
  }

  if (lastFb != 0 && millis() - lastFb > FB_TIMEOUT_MS) {
    Out.printf("!! 피드백 %lums 끊김\n", millis() - lastFb);
    testStop("피드백 두절");
  }
}

void testStart() {
  Out.println(">>> 시작 준비 — 손을 치우세요");
  ...
  lastStep = millis();
  running = true;
  Out.printf(">>> RUN  진폭 ±%.0f°  속도 %.0f°/s  Kp %.1f  Kd %.2f\n",
                SWEEP_DEG, SWEEP_RATE, MOTOR_KP, MOTOR_KD);
  Out.println("    멈추려면 stop");
}
...
void testStop(const char *reason) {
  ...
  bothUniversal(0xFD);
  target = 0;
  Out.printf(">>> STOP — %s  (피드백 %lu회)\n", reason, fb_count);
}

void printStatus() {
  Out.printf("[상태] %s  진폭±%.0f°  속도%.0f°/s  Kp%.1f Kd%.2f\n",
                running ? "RUN " : "STOP", SWEEP_DEG, SWEEP_RATE, MOTOR_KP, MOTOR_KD);
  Out.printf("       목표 %+.2f°   바깥(id%u) %s %+.2f°   안쪽(id%u) %s %+.2f°   피드백 %lu\n",
                target,
  ...
  if (canOk && twai_get_status_info(&ts) == ESP_OK)
    cs = (ts.state == TWAI_STATE_RUNNING) ? "RUNNING" : (ts.state == TWAI_STATE_BUS_OFF) ? "BUS_OFF" :
         (ts.state == TWAI_STATE_RECOVERING) ? "RECOVERING" : "STOPPED";
  Out.printf("       CAN %s  TX오류 %lu  RX오류 %lu  와이파이 %s\n", cs,
             (unsigned long)ts.tx_error_counter, (unsigned long)ts.rx_error_counter,
...
/* USB 와 TCP 를 문자 단위로 모은다. 한 줄씩 기다리는 방식은 개행까지 최대 1초를
 * 막아 그동안 CAN 을 못 비운다. 줄이 완성될 때만 handleCommand 로 넘긴다.
 */
void handleSerial() {
  static char ub[64]; static size_t un = 0;
  static char tb[64]; static size_t tn = 0;

  if (tcpServer.hasClient()) {
    if (tcpClient && tcpClient.connected()) tcpServer.available().stop();
    else { tcpClient = tcpServer.available(); tcpClient.setNoDelay(true); tn = 0;
           Out.printf(">>> TCP 접속 %s\n", tcpClient.remoteIP().toString().c_str()); }
  }
  while (Serial.available()) {
    char c = Serial.read();
    if (c == '\n' || c == '\r') { if (un) { ub[un] = 0; un = 0; handleCommand(String(ub)); } }
    else if (un < sizeof(ub) - 1) ub[un++] = c;
  }
  while (tcpClient && tcpClient.connected() && tcpClient.available()) {
    char c = tcpClient.read();
    if (c == '\n' || c == '\r') { if (tn) { tb[tn] = 0; tn = 0; handleCommand(String(tb)); } }
    else if (tn < sizeof(tb) - 1) tb[tn++] = c;
  }
}
...
void handleCommand(const String &raw) {
  ...
  if (u == "run") {
    if (running) Out.println("이미 돌고 있습니다");
    else testStart();
  } else if (u == "stop") {
    testStop("사용자 명령");
  } else if (u == "z") {
    ...
    Out.printf(">>> Kp = %.2f  (상한 %.0f)  진폭에서 약 %.2f N·m\n",
...
void setup() {
  ...
  delay(100);
  if (canOk) bothUniversal(0xFD);       // 부팅 시 비활성
  ...
  ArduinoOTA.onStart([]() { testStop("OTA 업로드"); });
  ArduinoOTA.begin();

  Out.printf("\n=== CAN 드라이버 %s  (install=%s, start=%s) ===\n",
             canOk ? "OK" : "!! 실패 — 모터 명령이 나가지 않습니다", esp_err_to_name(e1), esp_err_to_name(e2));
  Out.printf("=== 와이파이 AP  SSID %s  비밀번호 %s  IP %s  TCP %d ===\n",
             WIFI_SSID, WIFI_PASS, WiFi.softAPIP().toString().c_str(), TCP_PORT);

  Out.println("\n==================================================");
  Out.println(" MIT 모드 모터 왕복 테스트");
  Out.println(" 드라이버 ControlMode 가 MIT 여야 동작합니다.");
  Out.println(" 모터는 비활성 상태입니다. 'run' 을 입력해야 움직입니다.");
  Out.println("--------------------------------------------------");
  Out.println("  run     왕복 시작 (stop 까지 계속)");
  Out.println("  stop    0° 로 되돌린 뒤 정지");
  Out.println("  z       영점 재설정");
  Out.println("  ?       상태 출력");
  Out.println("--------------------------------------------------");
  Out.println("  값 조절            현재값    범위       예");
  Out.printf("  a<값>  진폭 [deg]  %6.1f    1 ~ %-3.0f    a15\n",  SWEEP_DEG,  SWEEP_DEG_MAX);
  Out.printf("  r<값>  속도 [deg/s]%6.1f    1 ~ %-3.0f    r60\n",  SWEEP_RATE, SWEEP_RATE_MAX);
  Out.printf("  kp<값> 강성        %6.2f    0 ~ %-3.0f    kp2\n",  MOTOR_KP,   MOTOR_KP_MAX);
  Out.printf("  kd<값> 감쇠        %6.2f    0 ~ %-3.0f    kd0.1\n", MOTOR_KD,   MOTOR_KD_MAX);
  Out.println("--------------------------------------------------");
  Out.printf("  지금 설정이면 한 번 왕복에 %.1f 초\n", 4.0f * SWEEP_DEG / SWEEP_RATE);
  Out.println("  빠르게: r60    크게: a15    부드럽게(시끄러움): kd0.1");
  Out.println("==================================================\n");
}
...
void loop() {
  ...
  if (now - lastPrint >= 100) {         // 10Hz
    lastPrint = now;
    Out.printf("target:%.2f,outer:%.2f,inner:%.2f\n",
