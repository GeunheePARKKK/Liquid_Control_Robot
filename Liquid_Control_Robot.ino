/*
 * Liquid_Control_Robot.ino  — 통합본 (2026-09-08)
 * -----------------------------------------------------------------------------
 *   = docs/CONTROL_INTEGRATION_TASK/[4단계] RTOS 태스크 분리/zv_shaping_rtos.ino
 *     (짐벌 트레이 제어: 수평 + 합력 벡터 + ZV + 피드포워드 + 수위 센서 로그)
 *   + step_control/drv8825_serial_control.ino
 *     (주행부 가변 서스펜션 스텝모터 2개, DRV8825 ×2)
 *
 * 트레이 제어 코드는 손대지 않았다. 스텝모터는 별도 태스크 susTask (코어0) 로
 * 붙였고 ctrlTask (코어1, 100Hz) 와 핀·버스를 공유하지 않는다.
 * 스텝모터 부분은 "가변 서스펜션" 으로 검색.
 *
 *   명령 : 1 서스펜션 딴딴하게 (강성 ↑, CW)    2 헐렁하게 (강성 ↓, CCW)
 *          0 서스펜션 정지                   + / - 스텝 속도
 *   배선 : #1 STEP 15 / DIR 16     #2 STEP 14 / DIR 21   (#2 는 DIR 반전)
 *          RESET·SLEEP → 3V3, GND 공통, VMOT 18~20V + 100µF, Vref 0.4V (0.8A)
 *
 * ---- 아래는 zv_shaping_rtos.ino 원문 헤더 ----
 * 합력벡터_제어.ino
 * -----------------------------------------------------------------------------
 ...
 *   서스펜션 : DRV8825 ×2, STEP 15/14, DIR 16/21 (통합본에서 추가)
 *
 * ─────────────────────────────────────────────────────────────────────────────
...
/* ── 가변 서스펜션 스텝모터 (2026-09-08, step_control/drv8825_serial_control.ino 이식) ──
 * 스텝모터 2개가 판스프링 고정부의 조절 나사를 돌려 서스펜션을 딴딴하게(강성 ↑)
 * 또는 헐렁하게(강성 ↓) 만든다. 두 모터는 같은 STEP 펄스를 같은 순간에 받는다.
 *
 *   1 -> 시계 방향(CW)   = 딴딴해지는 방향
 *   2 -> 반시계(CCW)     = 헐렁해지는 방향
 *   0 -> 정지            + / - -> 속도
 *
 * #2 모터는 배선상 #1 과 반대로 돌아서 DIR 을 뒤집는다 (SUS_DIR2_INVERT 1, 실측).
 * 4·5 는 CAN, 1·2 는 수위 센서, 11·12 는 I2C 가 쓴다.
 * DRV8825 Vref 는 0.4V (0.8A). 1.6V 로 두면 3.2A 가 되어 드라이버가 타고 연기 난다 (실측).
 */
#define SUS_STEP1_PIN 15
#define SUS_DIR1_PIN  16
#define SUS_STEP2_PIN 14
#define SUS_DIR2_PIN  21
#define SUS_DIR2_INVERT 1         // 두 모터가 서로 반대로 돌면 1

#define SUS_STEPS_PER_REV 200     // 1.8° 풀스텝
#define SUS_PULSE_US        5     // STEP 펄스 폭 (DRV8825 최소 1.9µs)
#define SUS_MIN_INTERVAL_US   3000    // 100 RPM 상한
#define SUS_MAX_INTERVAL_US 300000    // 1 RPM 하한
#define SUS_REVERSE_PAUSE_MS   100    // 방향 전환 시 잠깐 정지 (탈조 방지)

volatile int      susMode       = 0;        // 0 정지, 1 딴딴(CW), 2 헐렁(CCW)
volatile uint32_t susIntervalUs = 50000;    // 6 RPM (1회전 10초). ioTask 가 쓰고 susTask 가 읽는다
volatile uint32_t susSteps      = 0;        // 마지막 구동 시작부터 내보낸 STEP 수
...
/* ── 가변 서스펜션 ── */
float susRpm() {
  return 60000000.0f / ((float)susIntervalUs * SUS_STEPS_PER_REV);
}

void susPrintSpeed() {
  Serial.printf(">>> 서스펜션 스텝 속도 %.1f RPM (1회전 %.1f초, 간격 %luus)\n",
                susRpm(), 60.0f / susRpm(), (unsigned long)susIntervalUs);
}

/* 코어0, ioTask 보다 한 단계 높은 우선순위. 하는 일은 STEP 핀 두 개를 같은 순간에
 * 올렸다 내리는 것뿐이다. I2C·CAN·ADC 는 만지지 않으므로 ctrlTask 와 무관하다.
 * 6 RPM 이면 50ms 마다 한 번이라 1ms 틱의 vTaskDelay 로 충분하고, 마지막 2ms 는
 * 바쁜 대기로 맞춘다. 방향이 바뀌면 100ms 쉬고 DIR 을 바꾼 뒤 다시 시작한다.
 */
void susTask(void *arg) {
  pinMode(SUS_STEP1_PIN, OUTPUT);  pinMode(SUS_DIR1_PIN, OUTPUT);
  pinMode(SUS_STEP2_PIN, OUTPUT);  pinMode(SUS_DIR2_PIN, OUTPUT);
  digitalWrite(SUS_STEP1_PIN, LOW);  digitalWrite(SUS_DIR1_PIN, LOW);
  digitalWrite(SUS_STEP2_PIN, LOW);  digitalWrite(SUS_DIR2_PIN, LOW);
  ...
  for (;;) {
    ...
    if (m != lastMode) {
      if (lastMode != 0) vTaskDelay(pdMS_TO_TICKS(SUS_REVERSE_PAUSE_MS));
      bool cw = (m == 1);
      digitalWrite(SUS_DIR1_PIN, cw ? HIGH : LOW);
      digitalWrite(SUS_DIR2_PIN, (cw != (bool)SUS_DIR2_INVERT) ? HIGH : LOW);
      delayMicroseconds(10);                       // DIR 셋업 시간
      lastMode = m;
      susSteps = 0;
      nextUs   = esp_timer_get_time();
    }
    int64_t wait = nextUs - esp_timer_get_time();
    if (wait > 2000) { vTaskDelay(pdMS_TO_TICKS((wait - 1000) / 1000)); continue; }
    if (wait > 0)    delayMicroseconds((uint32_t)wait);
    else if (wait < -(int64_t)susIntervalUs) nextUs = esp_timer_get_time();   // 크게 밀렸으면 몰아치지 않는다

    digitalWrite(SUS_STEP1_PIN, HIGH);
    digitalWrite(SUS_STEP2_PIN, HIGH);
    delayMicroseconds(SUS_PULSE_US);
    digitalWrite(SUS_STEP1_PIN, LOW);
    digitalWrite(SUS_STEP2_PIN, LOW);
    susSteps = susSteps + 1;
    nextUs  += susIntervalUs;
  }
}
...
void printStatus() {
  ...
  Serial.printf("       서스펜션 %s   %.1f RPM   누적 %lu스텝   (1 딴딴 / 2 헐렁 / 0 정지 / + - 속도)\n",
                susMode == 0 ? "정지" : (susMode == 1 ? "딴딴하게 (강성 UP, CW)" : "헐렁하게 (강성 DOWN, CCW)"),
                susRpm(), (unsigned long)susSteps);
...
void handleLine(const char *raw) {
  ...
  /* ff / fj / fjr 은 f(CMD_LPF) 보다 먼저 검사해야 한다. */
  ...
  } else if (u == "1" || u == "2") {          // 가변 서스펜션
    int m = (u == "1") ? 1 : 2;
    susMode = m;
    Serial.printf(">>> 서스펜션 %s\n",
                  m == 1 ? "딴딴하게 (강성 UP)   CW  시계 방향" : "헐렁하게 (강성 DOWN) CCW 반시계 방향");
    susPrintSpeed();
  } else if (u == "0") {
    susMode = 0;
    Serial.println(">>> 서스펜션 정지");
  } else if (u == "+" || u == "=") {
    uint32_t v = susIntervalUs / 2;
    susIntervalUs = (v < SUS_MIN_INTERVAL_US) ? SUS_MIN_INTERVAL_US : v;
    susPrintSpeed();
  } else if (u == "-") {
    uint32_t v = susIntervalUs * 2;
    susIntervalUs = (v > SUS_MAX_INTERVAL_US) ? SUS_MAX_INTERVAL_US : v;
    susPrintSpeed();
  } else if (u == "q") {
    stream = !stream;
    ...
    Serial.println("명령: arm/stop/z/t/q/g/ag/ad/kp/kd/f/r/d/dp/dr/ff/fj/fjr/zv/zf/zd/zm/sr/sra/sre/srd/srh/srr/?   서스펜션: 1/2/0/+/-");
  }
}
...
void setup() {
  ...
  Serial.println("  [가변 서스펜션]  1 딴딴하게(강성 UP)  2 헐렁하게(강성 DOWN)  0 정지  +/- 속도");
  Serial.printf ("                   DRV8825 x2  STEP %d/%d  DIR %d/%d   %.1f RPM\n",
                 SUS_STEP1_PIN, SUS_STEP2_PIN, SUS_DIR1_PIN, SUS_DIR2_PIN, susRpm());
  Serial.println("==================================================\n");
  ...
  xTaskCreatePinnedToCore(susTask,  "sus",  3072, NULL, 3, NULL, 0);   // 가변 서스펜션 스텝모터
}
