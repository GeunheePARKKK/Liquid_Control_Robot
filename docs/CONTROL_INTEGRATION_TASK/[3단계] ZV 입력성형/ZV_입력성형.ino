 ...
 *
 *   제어 게인                        기본값      범위        올릴 때
 *     g<값>   수평 게인              ★ 0.30     0 ~ 1.5     g1.0 이 검증된 값
 *     ag<값>  합력 게인              ★ 0.00     0 ~ 1.5     0 이면 1단계와 동일
 ...
 */
float DIR_PITCH = -1.0f;    // [dp] 기본 −1   안쪽축 0x02
float DIR_ROLL  = +1.0f;    // [dr] 기본 +1   바깥축 0x01

// ── 제어 파라미터 ──
float GAIN            = 0.3f;    // [g]  기본 0.30   g1.0 이 검증된 값
const float LIMIT_DEG = 25.0f;   //      기본 25°    지령 각도 상한 (명령 없음)
float MAX_RATE        = 30.0f;   // [r]  기본 30°/s  ag 가 안 먹으면 여기부터
float CMD_LPF         = 0.15f;   // [f]  기본 0.15   클수록 빠르고 거칠다
int   CAN_DIV         = 2;       // [d]  기본 2      100Hz / 2 = 50Hz 전송
 ...
 *   것이라 원리적으로 못 없앤다.
 */
float MOTOR_KP = 1.0f;           // [kp] 기본 1.0    3 은 Kd 0 에서 발산했다
float MOTOR_KD = 0.1f;           // [kd] 기본 0.10   0 은 매뉴얼상 금지
 ...
 */
float ACC_GAIN = 0.0f;      // [ag] 기본 0.00  ★ 0 이면 1단계와 완전히 동일
float DIR_ACC  = +1.0f;     //      기본 +1    실측 확정 (명령 없음)
float ACC_LPF  = 0.2f;      // [ad] 기본 0.20  작을수록 느리고 매끈
 ...
 */
bool  ZV_ON   = false;      // [zv] 기본 꺼짐  ★ 꺼두면 2단계와 완전히 동일
float ZV_FREQ = 2.40f;      // [zf] 기본 2.40Hz  ★ 실측값으로 바꿀 것
float ZV_ZETA = 0.02f;      // [zd] 기본 0.02    액체는 보통 0.01~0.05
int   ZV_MODE = 2;          // [zm] 기본 2       2=ZV, 3=ZVD
...
void handleSerial() {
  ...
  if (u == "arm") {
    ...
    ZV_ON = (s.substring(2).toInt() != 0);
    if (ZV_ON) zvClear();          // 낡은 값이 섞여 첫 순간 튀는 것을 막는다
...
void loop() {
  ...
  if (stream && millis() - lastPrint > 20) {      // 50Hz
    lastPrint = millis();
    Serial.printf("pitch:%.2f,ref_pitch:%.2f,zv_pitch:%.2f,cmd_pitch:%.2f,act_pitch:%.2f,"
                  "roll:%.2f,ref_roll:%.2f,zv_roll:%.2f,cmd_roll:%.2f,act_roll:%.2f\n",
                  pitch_filtered, ref_pitch, shaped_pitch, cmd_pitch,
                  act_deg[CAN_ID_PITCH & 0x03] * DIR_PITCH,
                  roll_filtered,  ref_roll,  shaped_roll,  cmd_roll,
