 ...
 */
/* 실측으로 잡은 값 (2026-08-27, MIT_안전점검.ino 로 탐색)
 *
 *   Kp 1     트레이를 잡는다. 더 올릴 필요 없었다.
 *            Kp 3 은 Kd 0 에서 발산했다.
 *   Kd 0.1   밀었다 놓으면 왕복 없이 복귀.
 *            0.05 는 1회 오버슛, 0 은 여러 번 왕복 후 마찰로 겨우 수렴.
 *
 * ⚠ Kd 를 조금이라도 주면 모터·프레임에 미세 진동이 생긴다.
 *   엔코더 속도 추정 노이즈에 Kd 가 곱해지는 것이라 원리적으로 못 없앤다.
 *   또 Kd 0.1 에서 반복해서 밀었을 때 고주파 휘파람("이이이잉")이 한 번
 *   발생했다. 재현되지 않았으나 경계 조건으로 보고 남겨둔다.
 *   운전 중 발생하면 게인을 낮추고, 재현되면 위치/속도 모드를 재검토한다.
 */
float MOTOR_KP = 1.0f;     // 0.44 rad 에서 약 0.44 N·m
float MOTOR_KD = 0.1f;
