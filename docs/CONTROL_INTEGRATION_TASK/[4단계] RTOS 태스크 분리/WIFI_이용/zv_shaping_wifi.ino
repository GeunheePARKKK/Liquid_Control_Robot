 ...
 *     zf<값>  슬로싱 주파수 [Hz]      ★ 2.00     0.8 ~ 8     실측 확정. 물 높이 바뀌면 다시 잴 것
...
// =============================================================================
//  와이파이 (2026-09-06)  —  zv_shaping_rtos.ino 의 무선판
// =============================================================================
/* 보드가 직접 와이파이(AP)를 띄운다. 공유기가 없어도 된다.
 *
 *   SSID  WIFI_SSID / 비밀번호 WIFI_PASS      →  노트북·휴대폰이 여기에 붙는다
 *   IP    192.168.4.1                        →  TCP 포트 TCP_PORT 에 접속
 *
 * 접속하면 시리얼 모니터에 흐르던 것과 같은 줄이 그대로 오간다. 명령도 같다.
 *   PuTTY:  Raw, 192.168.4.1, 23
 *   파이썬: capture_wifi.py --host 192.168.4.1 ...
 *   휴대폰: TCP 터미널 앱
 *
 * USB 시리얼은 그대로 살아 있다. 출력은 둘 다에 나가고, 입력은 어느 쪽에서든
 * 받는다. 시리얼 모니터와 TCP 를 동시에 열어도 된다.
 *
 * 구조 — 왜 ctrlTask 에 영향이 없나
 *   Serial 을 만지는 곳은 원래부터 ioTask 하나였다 (handleLine, printStatus,
 *   샘플·이벤트 출력 전부). ctrlTask 는 qprintf 로 큐에 넣기만 한다.
 *   그래서 출력 대상을 "USB + TCP" 로 바꾸는 것도 ioTask 안에서만 일어난다.
 *   와이파이 스택은 core 0 에서 돌고 ioTask 도 core 0 이다. ctrlTask(core 1,
 *   100Hz) 는 손대지 않는다.
 *
 * TCP 로 쓸 때 막히지 않게 send(MSG_DONTWAIT) 를 쓴다. 상대가 느려서 버퍼가
 * 차면 그 줄은 버린다(dgTcpDrop). ioTask 가 몇 초씩 멈추는 것보다 낫다.
 *
 * OTA — 코드 업로드도 무선으로
 *   Arduino IDE 포트 목록에 "liquid-robot at 192.168.4.1" 이 뜬다 (노트북이
 *   이 AP 에 붙어 있을 때). 업로드가 시작되면 모터를 먼저 끈다.
 *   arduino-cli:  upload --port 192.168.4.1 --protocol network
 *
 * 전원 — 와이파이 송신 순간 3.3V 에서 최대 500mA 를 당긴다. 5V ≥ 1A 를
 * 보드 5V/VIN 핀에 넣어 내장 레귤레이터를 쓸 것. 외부 3.3V 소형 레귤레이터로
 * 직접 넣으면 전송 때마다 처져서 리셋되거나 CAN 트랜시버가 브라운아웃된다.
 *
 * TCP 로 접속해도 보드는 리셋되지 않는다 (USB 와 다름). 걸어둔 설정이 남는다.
 */
#define WIFI_SSID   "Liquid_Robot"
 ...
 public:
  ...
  size_t write(const uint8_t *b, size_t n) override {
    ...
    if (tcpClient && tcpClient.connected()) {
      int r = send(tcpClient.fd(), b, n, MSG_DONTWAIT);
      if (r < (int)n) dgTcpDrop += (r < 0) ? n : (n - r);
    }
    return n;
  }
};
 ...
 *   먼저 줄여놓고 켤 것.
 *
 * ZV_FREQ 2.00 은 실측값이다 (2026-09-04). 카트 위에서 트레이를 g0 ag0 으로
 *   고정한 채 밀어 수면을 셌다 — 1초에 2회 왕복. 책상 계산값 2.19 보다 9% 낮은데
 *   짐벌이 완전 강체가 아니라 결합계 주파수가 내려간 것으로 본다.
 *   슬로싱 주파수는 용기 형상과 액체 높이에 따라 변하므로 그것이 바뀌면 다시 잰다.
 *     직육면체 1차 모드 :  w^2 = (pi g/L) tanh(pi h/L)
 */
volatile bool  ZV_ON   = true;       // [zv] 기본 켜짐  ★ 실측 확정 (2026-09-04). zv0 이면 2단계와 동일
volatile float ZV_FREQ = 2.00f;      // [zf] 실측 확정 2.00Hz  카트 위 고정 트레이, 11x11cm 350mL (2026-09-04)
...
bool ctrlSend(uint8_t c) {
  ...
  Out.println("!! 명령 큐 가득 — 무시됨");   // 조용히 잃는 것보다 낫다
  return false;
}
...
void reportKp() {
  ...
  Out.printf(">>> Kp  안쪽 %.2f  바깥 %.2f  [N/rad]\n", KP_PITCH, KP_ROLL);
  Out.printf("    %.0f도 오차에서 토크  안쪽 %.2f  바깥 %.2f N·m\n", LIMIT_DEG, tp, tr);
  if (tp > TORQUE_WARN || tr > TORQUE_WARN)
    Out.printf("    !! %.1f N·m 초과 — 목표가 튀면 그만큼 나갑니다\n", TORQUE_WARN);
}

void reportKd(float changed) {
  Out.printf(">>> Kd  안쪽 %.3f  바깥 %.3f  [N·s/rad]\n", KD_PITCH, KD_ROLL);
  if (changed == 0.0f) Out.println("    !! Kd=0 은 매뉴얼상 발진·폭주 조건입니다");
  /* Kp 를 올리면 감쇠비가 sqrt(Kp) 만큼 떨어진다. 같이 안 올리면 발진한다. */
  if (KP_ROLL > 3.0f && KD_ROLL < 0.17f)
    Out.println("    !! 바깥축 Kp 대비 Kd 가 낮습니다 (1.75Hz 한계진동 이력)");
}

void printStatus() {
  Out.printf("[상태] %s  GAIN=%.2f  DIR(p/r)=%+.0f/%+.0f  IMU=%s\n",
                armed ? "ARMED" : "STOP", GAIN,
                DIR_PITCH, DIR_ROLL, imu_ok ? "OK" : "FAULT");
  Out.printf("       Kp 안쪽 %.2f / 바깥 %.2f    Kd 안쪽 %.3f / 바깥 %.3f\n",
                KP_PITCH, KP_ROLL, KD_PITCH, KD_ROLL);
  Out.printf("       피드포워드 %s   J %.4f/%.4f   v_des %+.0f°/s  t_ff %+.2f N·m\n",
                FF_ON ? "ON " : "OFF", FF_J_PITCH, FF_J_ROLL, ff_vel_p,
                constrain(FF_J_PITCH * ff_acc_p * DEG_TO_RAD, -FF_TMAX, FF_TMAX));
  Out.printf("       ACC_GAIN=%.2f  DIR_ACC=%+.0f  ACC_LPF=%.2f   ref(p/r)=%.2f/%.2f\n",
                ACC_GAIN, DIR_ACC, ACC_LPF, ref_pitch, ref_roll);
  /* 어느 성형기가 켜져 있는지 한눈에. 둘은 동시에 켜지지 않는다. */
  Out.printf("       ZV %s   %.2fHz  z=%.3f  %d임펄스  지연 +%.0fms\n",
                ZV_ON ? "ON " : "OFF", ZV_FREQ, ZV_ZETA, ZV_MODE, zvDelayMs());
  /* 값마다 어느 명령으로 바꾸는지 이름을 붙여 찍는다. 표를 다시 찾아보지
   * 않고 바로 고칠 수 있다.
   */
  Out.printf("       합력각(p/r) %.2f/%.2f°   자세보정게이트 %s/%s\n",
                force_pitch, force_roll,
                accGatePitch.active ? "BLOCK" : "TRACK",
                accGateRoll.active  ? "BLOCK" : "TRACK");
  Out.printf("       수위센서 %d   (마름 ~1070 / 잠김 ~2050. 정지 시 1400~1600 이 되게 높이 조절)\n", lvl_med);
  Out.printf("       와이파이 AP %s  IP %s  TCP %s  버림 %lu바이트\n",
             WIFI_SSID, WiFi.softAPIP().toString().c_str(),
             (tcpClient && tcpClient.connected()) ? tcpClient.remoteIP().toString().c_str() : "미접속",
             (unsigned long)dgTcpDrop);
  Out.printf("       소프트복귀 %s   sra %.2f°  sre %.2f°\n",
                SOFT_RETURN_ON ? "ON " : "OFF", SR_ACTIVE_DEG, SR_END_DEG);
  Out.printf("                       srd %ums(확인)  srh %ums(유지)  srr %ums(복귀)\n",
                SR_DWELL_MS, SR_HOLD_MS, SR_RETURN_MS);
  Out.printf("       소프트복귀 상태  안쪽 %s / 바깥 %s\n",
                softReturnPhaseName(srPitch.phase), softReturnPhaseName(srRoll.phase));
  Out.printf("       pitch=%.2f roll=%.2f  cmd(p/r)=%.2f/%.2f  act(p/r)=%.2f/%.2f\n",
                pitch_filtered, roll_filtered, cmd_pitch, cmd_roll,
                act_deg[CAN_ID_PITCH & 0x03], act_deg[CAN_ID_ROLL & 0x03]);
  Out.printf("       피드백 %lu회  I2C오류 %lu회  P(id%u)=%s  R(id%u)=%s\n",
                fb_count, i2c_err,
                CAN_ID_PITCH, statusName(last_status[CAN_ID_PITCH & 0x03]),
                CAN_ID_ROLL,  statusName(last_status[CAN_ID_ROLL  & 0x03]));
  if (fb_count == 0) {
    Out.println("       !! 피드백 0회 — ControlMode 가 MIT 인지, CAN 배선 확인");
  }
}
...
void handleLine(const char *raw) {
  ...
  /* ff / fj / fjr 은 f(CMD_LPF) 보다 먼저 검사해야 한다. */
    ...
    else Out.println("!! fjr 범위 0 ~ 0.1");
  } else if (u.startsWith("fj")) {
    float v = s.substring(2).toFloat();
    if (v >= 0.0f && v <= 0.1f) { FF_J_PITCH = v; Out.printf(">>> FF_J_PITCH = %.4f kg·m²\n", FF_J_PITCH); }
    else Out.println("!! fj 범위 0 ~ 0.1");
  } else if (u.startsWith("ff")) {
    FF_ON = (s.substring(2).toInt() != 0);
    Out.printf(">>> 피드포워드 %s   J 안쪽 %.4f / 바깥 %.4f   t_ff 상한 %.1f N·m\n",
    ...
    if (v >= 0.0f && v <= 1.5f) {
      ACC_GAIN = v;
      Out.printf(">>> ACC_GAIN = %.2f  (합력 벡터 %s)\n",
                    ACC_GAIN, ACC_GAIN == 0.0f ? "꺼짐 — 1단계와 동일" : "켜짐");
      if (ACC_GAIN > 0.0f)
        Out.println("   부호를 확인했습니까? 반대면 액체를 더 흔듭니다");
    } else Out.println("!! ACC_GAIN 범위 0 ~ 1.5");
  } else if (u.startsWith("ad")) {
    float v = s.substring(2).toFloat();
    if (v > 0.0f && v <= 1.0f) { ACC_LPF = v; Out.printf(">>> ACC_LPF = %.2f\n", ACC_LPF); }
    else Out.println("!! ACC_LPF 범위 0 ~ 1");
    ...
    d = -d;
    Out.printf(">>> DIR_%s = %+.0f  (%s축)\n",
                  isP ? "PITCH" : "ROLL", d, isP ? "안쪽" : "바깥");
    Out.println("    기울였을 때 트레이가 반대로 돌면 맞습니다");
    if (armed) Out.println("    !! ARMED 상태 — 지령이 반대편으로 이동합니다");
  } else if (u.startsWith("sra")) {
    float v = s.substring(3).toFloat();
    if (v >= 0.2f && v <= 10.0f && v > SR_END_DEG) {
      SR_ACTIVE_DEG = v;  srResetRequest = true;
      Out.printf(">>> SOFT_RETURN active = %.2f°\n", SR_ACTIVE_DEG);
    } else Out.println("!! sra 범위 0.2 ~ 10°, sre 보다 커야 합니다");
  } else if (u.startsWith("sre")) {
    float v = s.substring(3).toFloat();
    if (v >= 0.05f && v < SR_ACTIVE_DEG) {
      SR_END_DEG = v;  srResetRequest = true;
      Out.printf(">>> SOFT_RETURN end = %.2f°\n", SR_END_DEG);
    } else Out.println("!! sre 범위 0.05° 이상, sra 보다 작아야 합니다");
  } else if (u.startsWith("srd")) {
    int v = s.substring(3).toInt();
    if (v >= 10 && v <= 200) {
      SR_DWELL_MS = (uint16_t)v;  srResetRequest = true;
      Out.printf(">>> SOFT_RETURN 종료 확인 = %ums\n", SR_DWELL_MS);
    } else Out.println("!! srd 범위 10 ~ 200ms");
  } else if (u.startsWith("srh")) {
    int v = s.substring(3).toInt();
    if (v >= 10 && v <= 500) {
      SR_HOLD_MS = (uint16_t)v;  srResetRequest = true;
      Out.printf(">>> SOFT_RETURN 유지 = %ums\n", SR_HOLD_MS);
    } else Out.println("!! srh 범위 10 ~ 500ms");
  } else if (u.startsWith("srr")) {
    int v = s.substring(3).toInt();
    if (v >= 20 && v <= 1000) {
      SR_RETURN_MS = (uint16_t)v;  srResetRequest = true;
      Out.printf(">>> SOFT_RETURN 복귀 = %ums\n", SR_RETURN_MS);
    } else Out.println("!! srr 범위 20 ~ 1000ms");
    ...
    ZV_UNLOCK();
    srResetRequest = true;
    Out.printf(">>> SOFT_RETURN %s  active/end=%.2f/%.2f°  dwell/hold/return=%u/%u/%ums%s\n",
    ...
    if (v >= 5.0f && v <= 400.0f) {
      MAX_RATE = v;
      Out.printf(">>> MAX_RATE = %.0f °/s  (%.0fms 안에 %.1f° 이동 가능)\n",
                    MAX_RATE, 110.0f, MAX_RATE * 0.11f);
      if (v > 200.0f)
        Out.println("   !! 지령이 모터보다 빨라집니다. 드룹·진동을 보세요");
    } else Out.println("!! MAX_RATE 범위 5 ~ 400");
  } else if (u.startsWith("g")) {
    float v = s.substring(1).toFloat();
    if (v >= 0.0f && v <= 1.5f) { GAIN = v; Out.printf(">>> GAIN = %.2f\n", GAIN); }
    else Out.println("!! GAIN 범위 0 ~ 1.5");
  } else if (u.startsWith("f")) {
    float v = s.substring(1).toFloat();
    if (v > 0.0f && v <= 1.0f) { CMD_LPF = v; Out.printf(">>> CMD_LPF = %.2f\n", CMD_LPF); }
    else Out.println("!! CMD_LPF 범위 0 ~ 1");
  } else if (u.startsWith("d")) {
    int v = s.substring(1).toInt();
    if (v >= 1 && v <= 20) { CAN_DIV = v; Out.printf(">>> CAN_DIV = %d (%dHz)\n", v, 100 / v); }
    else Out.println("!! CAN_DIV 범위 1 ~ 20");
  } else if (u == "q") {
    stream = !stream;
    Out.printf(">>> 스트림 출력 %s\n", stream ? "ON" : "OFF");
  } else if (u == "?") {
    ...
    Out.println("명령: arm/stop/z/t/q/g/ag/ad/kp/kd/f/r/d/dp/dr/ff/fj/fjr/zv/zf/zd/zm/sr/sra/sre/srd/srh/srr/?");
  }
}
...
void setup() {
  ...
  uint8_t id = readReg(REG_WHO_AM_I);
  Out.printf("WHO_AM_I: 0x%02X (기대값 0x%02X)  주소 0x%02X\n",
                id, WHO_AM_I_VAL, IMU_ADDR);
  imu_ok = (id == WHO_AM_I_VAL);
  if (!imu_ok) {
    Out.println("!! IMU 응답 없음 — CS→3V3, SDO→GND 인지, SDA/SCL 풀업 확인");
  }
  configSensor();

  // ── 자이로 영점 + 중력 스케일 ──
  Out.println(">>> 캘리브레이션 중... 움직이지 마세요");
  long  sgx = 0, sgy = 0;
  float smag = 0;
  for (int i = 0; i < 200; i++) {
  ...
  Out.printf("gyro bias x:%.1f y:%.1f   g_scale:%.4f   I2C오류 %lu회\n",
  ...
  lastLoop = micros();
  Out.println("\n==================================================");
  Out.println(" 2단계 — 합력 벡터 목표각");
  Out.println(" IMU 는 I2C, 모터는 MIT 모드입니다.");
  Out.println(" 모터는 비활성 상태입니다. 'arm' 을 입력해야 움직입니다.");
  Out.println("--------------------------------------------------");
  Out.println("  arm     활성화        stop  비활성화");
  Out.println("  t       시험 구동     z     영점 재설정");
  Out.println("  g<값>   수평 게인     예) g1.0");
  Out.printf("  ag<값>  합력 게인     예) ag0.3   현재 %.2f%s\n",
                 ACC_GAIN, ACC_GAIN == 0.0f ? "  ← 0 이면 1단계와 동일" : "");
  Out.println("  ad<값>  합력 필터     예) ad0.2");
  Out.println("  kp<값>  모터 강성     kd<값>  모터 감쇠   (두 축 함께)");
  Out.printf("  kpp/kpr kdp/kdr 축별   현재 Kp %.2f/%.2f  Kd %.3f/%.3f (안/바깥)\n",
                 KP_PITCH, KP_ROLL, KD_PITCH, KD_ROLL);
  Out.println("  f<값>   지령 필터     d<값>   CAN 분주     ?  상태");
  Out.printf("  r<값>   슬루 제한     예) r120    현재 %.0f °/s\n", MAX_RATE);
  Out.printf("  zv1/zv0 ZV 켜기/끄기  zf<값> 주파수  zd<값> 감쇠비  zm<2|3>\n");
  Out.printf("          현재 %s  %.2fHz  z=%.3f  %d임펄스  지연 +%.0fms\n",
                 ZV_ON ? "켜짐" : "꺼짐 ← 꺼두면 2단계와 동일",
                 ZV_FREQ, ZV_ZETA, ZV_MODE, zvDelayMs());
  Out.printf("  dp / dr 부호 뒤집기   현재 %+.0f / %+.0f  (안쪽 / 바깥)\n",
                 DIR_PITCH, DIR_ROLL);
  Out.printf("  ff1/ff0 모터 피드포워드   fj/fjr 관성   현재 %s  J %.4f/%.4f\n",
  ...
  xTaskCreatePinnedToCore(ioTask,   "io",   8192, NULL, 2, NULL, 0);   // 와이파이·OTA 몫으로 +2K
}
...
void ioTask(void *arg) {
  char line[96];   size_t n = 0;    // USB 줄 버퍼
  char tline[96];  size_t tn = 0;   // TCP 줄 버퍼 — 두 입력이 한 줄에 섞이지 않게
  char txt[EVT_LEN];
  Snap sn;
  ...
  for (;;) {
    ...
    while (xQueueReceive(qEvt, txt, 0) == pdTRUE) Out.print(txt);
    ...
    // TCP 입력. 같은 파서, 같은 명령.
    while (!ctrlBusy && tcpClient && tcpClient.connected() && tcpClient.available()) {
      char c = tcpClient.read();
      if (c == '\n' || c == '\r') {
        if (tn) { tline[tn] = 0;  tn = 0;  handleLine(tline); }
      } else if (tn < sizeof(tline) - 1) {
        tline[tn++] = c;
      }
    }
  }
