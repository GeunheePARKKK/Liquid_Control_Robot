...
void printStatus() {
  ...
  Serial.printf("       소프트복귀 %s  active/end=%.2f/%.2f°  %u+%ums  상태=%s/%s\n",
                SOFT_RETURN_ON ? "ON " : "OFF", SR_ACTIVE_DEG, SR_END_DEG,
                SR_HOLD_MS, SR_RETURN_MS,
...
void setup() {
  ...
  Serial.printf ("          현재 %s  %.2fHz  z=%.3f  %d임펄스  지연 +%.0fms\n",
                 ...
                 ZV_FREQ, ZV_ZETA, ZV_MODE, zvDelayMs());
  Serial.printf ("  sr1/sr0 소프트복귀    sra/sre 임계각  srd/srh/srr 시간(ms)\n");
  Serial.printf ("          현재 %s  %.2f/%.2f°  %u/%u/%ums (확인/유지/복귀)\n",
                 SOFT_RETURN_ON ? "켜짐" : "꺼짐", SR_ACTIVE_DEG, SR_END_DEG,
                 SR_DWELL_MS, SR_HOLD_MS, SR_RETURN_MS);
...
void ctrlTask(void *arg) {
  ...
  for (;;) {
    ...
    /* ── 3.5. 목표각 성형 ──
     * ZV 는 링 버퍼의 반주기 전 값을 섞고, 소프트 복귀는 가속 종료 구간만
     * HOLD→최소저크 RETURN 으로 바꾼다. 둘은 동시에 사용하지 않는다.
